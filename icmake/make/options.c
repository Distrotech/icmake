...
int options(char **argv, int *argc)
{
    ...
    while ((c = icm_getopt(argc, argv)) != -1)
    {
        switch (c)
        {
            case 'a':
            ...
            case 'C':
                if (!(g_config_name = icm_getoptval(argc, argv)))
                    error("-C requires configuration file path");
                g_config_name = xstrdup(g_config_name);
            break;

            case 'i':
                g_flags |= f_icmake; /* flag icmake taken literally  */
                if (!(g_source_name = icm_getoptval(argc, argv)))
                    error("-i requires source-filename");
                return icm_getoptindex();   /* and return the index of args */
                                            /* to icm-exec */
            case 'p':
            ...
            return icm_getoptindex() + 1;   /* index of remaining args */
                                            /* + 1 for the extra arg on the */
                                            /* #!/...-x... line */

            case '-':
                return icm_getoptindex();   /* return index of args to icm-exec
                                            */
        }
    }
